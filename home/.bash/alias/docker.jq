...
(
  "alias \"\(.key)-ps\"=\"docker ps --all --filter \\\"name=\(.name)\\\"\"",
  (
    { key, value: .value[], name } |
    (
      "alias \"\(.key)-\(.value)-ps\"=\"docker ps --all --filter \\\"name=\(.name)-\(.value)\\\"\"",
      "alias \"\(.key)-\(.value)\"=\"docker ps --all --filter \\\"name=\(.name)-\(.value)\\\" --quiet | xargs docker\"",
